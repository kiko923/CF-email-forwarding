...
它将 Cloudflare 原本仅限单人使用的 Email Routing 底层能力进行了整合。站长或团队可以零成本搭建一个供多人协作的虚拟邮箱分配平台。用户只需在控制台点按几下，即可生成带有专属域名（含子域名）的虚拟邮箱，所有邮件将极速、安全、无痕地穿透转发至用户的真实邮箱（如 QQ、网易、Gmail 等）。
